...
if (!isset($_POST['buyNX'])) {
    ...
    $fetchChar = $mysqli->query("SELECT * FROM `characters` WHERE `accountid` = '".$_SESSION['id']."'") or exit();
    ...
    $hasMeso = $mysqli->query("SELECT * FROM `characters` WHERE `id` = '".$selChar."'") or exit();
    $getMeso = $hasMeso->fetch_assoc();
    $fetchNX = $mysqli->query('SELECT * FROM `'.$prefix."buynx` WHERE `meso` = '".$selPack."'") or exit();
    ...
    if (empty($selChar)) {
        ...
        $fetchCharId = $mysqli->query("SELECT * FROM `characters` WHERE `id` = '".$selChar."'") or exit();
        $getCharId = $fetchCharId->fetch_assoc();
        $mysqli->query('UPDATE `characters` SET `meso` = meso - '.$selPack.' WHERE `id` = '.$selChar.'') or exit();
        $mysqli->query("UPDATE `accounts` SET $colnx = $colnx + ".$selNX['nx'].' WHERE `id` = '.$getCharId['accountid'].'') or exit();
