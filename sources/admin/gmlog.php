...
try {
    ...
    while ($row = $query->fetch_assoc()) {
        ...
        if (!array_key_exists('command', $row) || $row['command'] == '') {
            $row['command'] = 'Unknown';
        }
        if (!array_key_exists('when', $row) || $row['when'] == '') {
            $row['when'] = 'Unknown';
        }
        ...
        $kill = '!kill';
        $dc = '!dc';
        if (strpos($row['command'], $warp) === false) {
            $warp_class = '';
        } else {
            $warp_class = 'info';
        }
        if (strpos($row['command'], $dc) === false) {
            $dc_class = '';
        } else {
            $dc_class = 'error';
        }
        if (strpos($row['command'], $kill) === false) {
            $jail_class = '';
        } else {
            $jail_class = 'warning';
        }
